...
4. Add contacts under **Settings → Contacts** and assign each their
   ...
   **A contact only hears about the validators you tick for them.** If you add
   a validator later, come back here and tick it — it does not attach itself
   to existing contacts. Anything no contact covers falls through to the
   fallback under "anything unassigned", and if you have not set one, nobody
   is told.
5. **Telegram notifications** need your own bot — alerts go through it and
   ...
   **Expect a message every morning.** Once anyone is paired, xxOps sends a
   summary at 08:00: how many hosts are reporting, anything that looks off, and
   whether the backup ran. It arrives whether or not anything is wrong, so a
   quiet morning still gets one — that is how you know the monitor itself is
   alive. Alerts are separate and arrive whenever something happens.

   To stop it: `sudo systemctl disable --now xxops-digest.timer` on the
   monitor. Alerts keep working.
...
files this script downloads before any of them are installed, and it
puts the host on the Commands tab so you can run actions from the app.
**The agent will not install without it.** If you only want metrics and
alerts, pass `--skip-agent` instead.
...
**That is the whole of Part 3.** Everything below is the same work done by hand — worth reading if you want to know what the script did, and worth
