...
if [ -f "$STATE_DIR/cmd_key" ] || [ -f /etc/xxops/cmd_key ]; then
  step "keep the existing agent signing key (never regenerated)"
  [ -f /etc/xxops/cmd_key ] && [ ! -f "$STATE_DIR/cmd_key" ] \
    && step "move it to $STATE_DIR/cmd_key, where the app reads it"
else
  step "generate an agent signing key at $STATE_DIR/cmd_key"
fi
...
# The agent installer itself. It is fetched over plain HTTP by the host
# installer, so it has to be servable AND hashable - the manifest lists it,
# and without this it would 404 and its line would be missing.
install -m 644 "$REPO/agent/install.sh" "$APP_DIR/install.sh"
...
# It lives in $STATE_DIR because that is where the app looks: XXOPS_CMD_KEY
# defaults to $STATE_DIR/cmd_key and nothing sets that variable. Earlier
# installs put it in /etc/xxops, where the app could neither find nor read it,
# and the Commands tab failed with "could not sign".
if [ -f /etc/xxops/cmd_key ] && [ ! -f "$STATE_DIR/cmd_key" ]; then
  # MOVED, never regenerated - a new key would orphan every agent that
  # already trusts the old one.
  mv /etc/xxops/cmd_key "$STATE_DIR/cmd_key"
  [ -f /etc/xxops/cmd_key.pub ] && mv /etc/xxops/cmd_key.pub "$STATE_DIR/cmd_key.pub"
  say "  moved the signing key to $STATE_DIR/cmd_key (same key, where the app reads it)"
fi

if [ ! -f "$STATE_DIR/cmd_key" ]; then
  ssh-keygen -t ed25519 -N "" -C "xxops-command-key" -f "$STATE_DIR/cmd_key" >/dev/null
  say "  generated $STATE_DIR/cmd_key"
...
# Outside the blocks above on purpose, so an install that already has the key
# with the wrong ownership is repaired by re-running. The app is not root.
chown "$RUN_USER":"$RUN_USER" "$STATE_DIR/cmd_key" "$STATE_DIR/cmd_key.pub"
chmod 600 "$STATE_DIR/cmd_key"
chmod 644 "$STATE_DIR/cmd_key.pub"
...
# $STATE_DIR first: that is where the key now lives, and on a FRESH install
# it is the only place it has ever been. The old path is a fallback for a
# monitor that has not been through the move yet. Reading only the old path
# meant a fresh monitor never wrote this file at all, and every agent install
# 404ed fetching it.
_PUB=""
if [ -f "$STATE_DIR/cmd_key.pub" ]; then
  _PUB="$STATE_DIR/cmd_key.pub"
elif [ -f /etc/xxops/cmd_key.pub ]; then
  _PUB=/etc/xxops/cmd_key.pub
fi
if [ -n "$_PUB" ]; then
  install -d -m 755 /etc/xxops
  printf 'xxops-monitor %s' "$(cat "$_PUB")" \
    > /etc/xxops/allowed_signers
  chmod 644 /etc/xxops/allowed_signers
  say "  wrote /etc/xxops/allowed_signers"
else
  echo "REFUSING: no signing key public half found, so allowed_signers" >&2
  echo "cannot be written and no agent will be able to install." >&2
  exit 1
fi
...
if [ ! -f /etc/xxops/enroll_token ]; then
  install -d -m 755 /etc/xxops
  ( umask 077; head -c 24 /dev/urandom | base64 | tr -d '/+=' \
      > /etc/xxops/enroll_token )
  say "  generated /etc/xxops/enroll_token"
fi
# Outside the block above on purpose: the app reads this file, and the app
# does NOT run as root. Left root-owned it is unreadable, and the Commands tab
# reports "no token yet" while the token sits there perfectly well formed.
# Correcting on every run also repairs an install that already has it wrong.
chown "$RUN_USER":"$RUN_USER" /etc/xxops/enroll_token
chmod 600 /etc/xxops/enroll_token
