...
/* The empty state: no host has registered yet. Held separately from cmdErr
   because it is not an error - it is the normal state of a new install, and
   it wants instructions rather than a red box. */
let cmdEmpty = false;
let enrollToken = null;
let enrollMsg = null;


function renderCommandsEmpty(monitorHost){
  const cmd =
    "curl -sL https://raw.githubusercontent.com/Bulldog-Master/xxOps/main/" +
    "fixes/xxops-host-install.sh \\\n  | sudo bash -s -- --label <THIS-HOST-LABEL>" +
    " --monitor " + monitorHost + " --token " +
    (enrollToken || "<TOKEN>");
  return "" +
    "<div class='empty'>" +
      "<h3>No hosts have registered yet</h3>" +
      "<p>A host appears here once its agent tells the monitor where to " +
      "reach it. That happens during install, when you pass the enrolment " +
      "token below.</p>" +
      (enrollToken
        ? "<p><b>Your enrolment token</b></p><pre class='token'>" +
          esc(enrollToken) + "</pre>"
        : "<p><b>" + esc(enrollMsg || "No enrolment token on this monitor.") +
          "</b></p>") +
      "<p>Run this on each node and gateway. It is safe to re-run on a host " +
      "that is already set up \u2014 that is also how you add one that was " +
      "installed before you had a token:</p>" +
      "<pre>" + esc(cmd) + "</pre>" +
      "<p class='muted'>Hosts still report metrics and raise alerts without " +
      "this. Registering only adds them here, so you can run actions from " +
      "the app instead of over SSH.</p>" +
    "</div>";
}
...
async function loadAgents(){
  ...
  agentsLoading = true; cmdErr = null; cmdEmpty = false;
  try{
    ...
    if(!Object.keys(probe).length){
      /* Not an error - no host has registered yet. Show what to do about it:
         the enrolment token, and the command that uses it. Fetched from an
         owner-only route, so this is only ever visible to the fleet owner. */
      cmdCat = {}; cmdErr = null;
      try{
        const t = await aget("/api/enroll-token");
        enrollToken = (t && t.ok) ? t.token : null;
        enrollMsg = (t && !t.ok) ? t.message : null;
      }catch(e){ enrollToken = null; enrollMsg = null; }
      cmdEmpty = true;
      return;
    }
...
function vCommands(){
  ...
  if(cmdEmpty)
    /* The address in the URL bar - by definition one that reaches the
       monitor from wherever the operator is standing. */
    return renderCommandsEmpty(location.hostname);
