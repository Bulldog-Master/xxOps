...
async function loadNotifyCfg(){
  try{
    ...
    // _rev travels with the state and goes back on save, so the server can
    // refuse a save built on a copy that is no longer current.
  }catch(e){ notify = null; }
}
async function saveNotifyCfg(){
  ...
  try{
    ...
    if(r.status === 409){ notifyMsg = "\u26A0 " + d.message; }
