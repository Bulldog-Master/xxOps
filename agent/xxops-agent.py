...
Python functions, so no text from the network becomes a command. That bounds
the SHAPE of what can run, not its consequence: the catalogue includes actions
that stop cMix, restart the gateway and chain, and run an upgrade-and-reboot.
Those go through exact sudoers lines and need an explicit confirm in the
signed request. If signature checking were bypassed, a catalogue name and a
confirm flag would be enough to restart a validator - the containment is the
signature and the fixed catalogue together, not the catalogue alone.

Requests are signed with ssh-keygen -Y, so agents hold only a PUBLIC key -
compromising one host cannot forge requests to any other. Each request is
bound to one host, expires, and carries a nonce that cannot be reused, kept
across restarts so a captured request cannot be replayed into a fresh
process.

Read actions and change actions. The changing ones - restarting or
stopping cMix, the gateway and the chain, and the update-and-reboot
scripts - run through exact sudoers lines and require an explicit
confirm in the signed request.
...
# Loopback unless told otherwise. The installer sets this to the mesh or
# default-route address; running this file by hand used to publish the command
# endpoint on every interface, including whatever faces the internet.
#
# It is also the answer to "/health is unauthenticated": registration needs
# /health to answer without a signature, because the monitor calls it to check
# what is really at a claimed address. Not being reachable beats answering
# politely to strangers.
BIND    = os.environ.get("XXOPS_AGENT_BIND", "127.0.0.1")
...
def a_disk_usage():
    ...
    """
    # /opt/xxnetwork is complete: the installer grants this account traverse
    # there. /var/lib and /var/log report only what it can already reach, so
    # those numbers are a floor rather than a total.
    #
    # That is deliberate. Making them complete would need either a
    # filesystem-wide read capability - which is what was removed, and the
    # reason it was removed - or sudo lines for du. Do not "fix" this by
    # restoring AmbientCapabilities.
    out = []
...
# Nonces already used, so a signed request cannot be replayed.
#
# PERSISTED, because in memory it was empty again after every restart - and
# the agent restarts on every update. A request captured inside its 60-second
# window could be replayed once across that gap. Not /run, which a reboot
# clears, and a reboot is precisely when this restarts.
NONCE_FILE = os.environ.get("XXOPS_AGENT_NONCES",
                            "/var/lib/xxops-agent/nonces")
...
def _save_seen(d):
    """Rewrite atomically. Best effort: a failure here must not stop the
    agent answering, it only shortens how far back replay protection reaches
    if the process dies."""
    try:
        os.makedirs(os.path.dirname(NONCE_FILE), exist_ok=True)
        tmp = NONCE_FILE + ".tmp"
        with open(tmp, "w") as fh:
            for nonce, when in d.items():
                fh.write("%s %s\n" % (when, nonce))
        os.chmod(tmp, 0o600)
...
class H(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        try:
            # Same cap as the monitor: a signed request is small, and
            # trusting a stranger's Content-Length is a memory DoS.
            try:
