#!/bin/bash
# Take all pending package updates on this gateway and reboot.
#
...
if [ "${1:-}" != "--go" ]; then
  ...
  echo "update started - this gateway will reboot when it finishes"
  echo "it drops off the network until it returns, usually a few minutes"
...
systemctl stop xxnetwork-gateway  || true
