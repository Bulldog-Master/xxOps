...
# Read from the environment, the same way xxops-server.py does. The unit
# sources /etc/xxops/xxops.conf, which already holds the monitor's address, so
# there is one source of truth and changing it needs no reinstall.
PROM = os.environ.get("XXOPS_PROM_URL", "http://127.0.0.1:9090")
...
try:
    ...
    # How many hosts this monitor has seen in the last day, rather
    # than a number baked in by whoever wrote this. A fleet of any
    # size measures itself.
    expected = q('count(count by (instance) (up{pilot="xxops"} offset 1d))') or 0
    expected = int(expected)
    lines.append("hosts reporting: %s of %d" %
                 (int(hosts) if hosts else 0, expected))
    if not hosts or int(hosts) < expected:
        healthy = False
