...
EXEMPT = {
    ...
    "/api/agent/register":
        "an agent is a daemon on another machine, not a browser, so it has "
        "no session and never will. It carries an enrolment token instead: "
        "the handler compares it with hmac.compare_digest and refuses "
        "everything if no token file exists, then calls the claimed address "
        "back on /health and stores what the agent says it is rather than "
        "what the caller claimed. Nothing is written before both checks pass.",
}
EXEMPT_PREFIXES = {
