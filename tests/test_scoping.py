...
class Scoping(unittest.TestCase):
    ...
    def resolutions(self, entries):
        """Put a resolutions file where the server reads it."""
        with open(self.mod.RESOLUTIONS, "w") as f:
            json.dump(entries, f)

    ALPHA = {"id": "a1", "host": "alpha", "title": "alpha note"}
    BRAVO = {"id": "b1", "host": "bravo", "title": "bravo note"}
    FLEET = {"id": "f1", "host": "", "title": "fleet note"}
    GW = {"id": "g1", "host": "alpha_gt", "title": "gateway note"}

    def alpha_contact(self):
        """A contact bound to alpha, whose gateway is paired in settings."""
        self.write_state(contacts=[self.contact("c1", ["alpha"])],
                         pairs={"alpha": "alpha_gt"})
        return {"role": "contact", "contactId": "c1"}

    # -- resolutions: what a contact may see ------------------------------

    def test_contact_sees_only_their_own_hosts(self):
        """The whole point. A note about someone else's box is not theirs."""
        me = self.alpha_contact()
        allowed = self.mod.allowed_instances(me)
        got = self.mod.scope_resolutions(
            [self.ALPHA, self.BRAVO, self.FLEET, self.GW], allowed)
        self.assertEqual([e["id"] for e in got], ["a1", "g1"])

    def test_contact_does_not_see_fleet_notes(self):
        """A note with no host describes the fleet, not one machine.

        Default deny: the safe reading of an unhosted note is that it is the
        operator's, not that it belongs to everyone.
        """
        me = self.alpha_contact()
        allowed = self.mod.allowed_instances(me)
        self.assertFalse(self.mod.may_touch_resolution(self.FLEET, allowed))

    def test_owner_sees_everything_including_fleet_notes(self):
        self.write_state()
        allowed = self.mod.allowed_instances({"role": "owner"})
        got = self.mod.scope_resolutions(
            [self.ALPHA, self.BRAVO, self.FLEET, self.GW], allowed)
        self.assertEqual(len(got), 4)

    def test_paired_gateway_comes_along(self):
        """Same rule as metrics: the gateway settings pairs to their node."""
        me = self.alpha_contact()
        allowed = self.mod.allowed_instances(me)
        self.assertTrue(self.mod.may_touch_resolution(self.GW, allowed))

    def test_host_is_stripped_on_both_sides(self):
        """A note stored as " alpha" must not become invisible to everyone
        and matchable by nothing."""
        me = self.alpha_contact()
        allowed = self.mod.allowed_instances(me)
        self.assertTrue(self.mod.may_touch_resolution(
            {"host": "  alpha  "}, allowed))

    def test_host_match_is_case_sensitive(self):
        """Prometheus labels are exact. Folding case here would let a note
        scope differently from the metrics it is about."""
        me = self.alpha_contact()
        allowed = self.mod.allowed_instances(me)
        self.assertFalse(self.mod.may_touch_resolution(
            {"host": "Alpha"}, allowed))

    def test_missing_host_key_is_not_a_wildcard(self):
        me = self.alpha_contact()
        allowed = self.mod.allowed_instances(me)
        self.assertFalse(self.mod.may_touch_resolution({}, allowed))
        self.assertFalse(self.mod.may_touch_resolution(None, allowed))

    def test_a_contact_with_no_validators_sees_nothing(self):
        """An empty set is not "unrestricted"."""
        self.write_state(contacts=[self.contact("c1", [])])
        allowed = self.mod.allowed_instances({"role": "contact",
                                              "contactId": "c1"})
        got = self.mod.scope_resolutions(
            [self.ALPHA, self.BRAVO, self.FLEET], allowed)
        self.assertEqual(got, [])

    # -- the same rule must hold on writes --------------------------------

    def test_contact_cannot_write_to_another_host(self):
        """add / edit / delete all run through may_touch_resolution, so this
        is the invariant every write path depends on."""
        me = self.alpha_contact()
        allowed = self.mod.allowed_instances(me)
        self.assertFalse(self.mod.may_touch_resolution(self.BRAVO, allowed))
        self.assertTrue(self.mod.may_touch_resolution(self.ALPHA, allowed))

    def test_claiming_a_host_you_own_does_not_grant_someone_elses_note(self):
        """The stored host decides, never the body.

        A contact editing b1 while sending host="alpha" must be judged on
        what is on disk, or they rewrite another contact's note by asserting
        it is theirs.
        """
        me = self.alpha_contact()
        allowed = self.mod.allowed_instances(me)
        stored = self.BRAVO                      # what is on disk
        claimed = {"id": "b1", "host": "alpha"}  # what they sent
        self.assertFalse(self.mod.may_touch_resolution(stored, allowed))
        self.assertTrue(self.mod.may_touch_resolution(claimed, allowed))
        # The handler checks `stored`. If it ever checks `claimed`, the two
        # lines above are the bug.
